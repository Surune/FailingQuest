...
public static class RouteChecks {
 ...
 GameManager.Instance.ResetRun();SceneManager.LoadScene("MapScene");yield return new WaitForSeconds(0.3f);
 var map=Map.MapManager.instance.map;var node=map.nodes.First(n=>n.point.x==0); map.userPath.Add(node.point);map.hasSelectedNode=true;
 ...
 map=Map.MapManager.instance.map;map.userPath.Add(map.GetBossNode().point);SceneManager.LoadScene("BattleScene");yield return new WaitForSeconds(0.3f);
 ...
 Check(SceneManager.GetActiveScene().name=="GameClearScene","Boss victory reaches clear scene");Check(GameManager.Instance.currentMap.nodes.Count==0,"Clear removes completed map");
